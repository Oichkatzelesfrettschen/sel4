...
static void unsetDr7BitsFor(tcb_t *t, uint16_t bp_num) {
  ...
  case 0:
    mask =
        (0x3u << X86_DEBUG_BP0_SIZE_SHIFT) | (0x3u << X86_DEBUG_BP0_TYPE_SHIFT);
    break;
  case 1:
    mask =
        (0x3u << X86_DEBUG_BP1_SIZE_SHIFT) | (0x3u << X86_DEBUG_BP1_TYPE_SHIFT);
    break;
  case 2:
    mask =
        (0x3u << X86_DEBUG_BP2_SIZE_SHIFT) | (0x3u << X86_DEBUG_BP2_TYPE_SHIFT);
    break;
  default: /* 3 */
    assert(bp_num == 3);
    mask =
        (0x3u << X86_DEBUG_BP3_SIZE_SHIFT) | (0x3u << X86_DEBUG_BP3_TYPE_SHIFT);
    break;
  }
...
/**
 * Disable a breakpoint without clearing its configuration.
 *
 * @param t       Target thread control block.
 * @param bp_num  Hardware breakpoint identifier (0..N).
 */
...
/**
 * Query whether a given breakpoint is enabled.
 *
 * @param t       Target thread control block.
 * @param bp_num  Hardware breakpoint identifier (0..N).
 *
 * @return True if the breakpoint is enabled; otherwise false.
 */
